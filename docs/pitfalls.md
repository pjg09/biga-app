...
- Un `EXCLUDE` **no admite `NOT VALID`** (solo `CHECK` y `FK`): no hay forma de añadirlo "a validar después". Si los datos existentes lo violan, el `ALTER TABLE` falla con un mensaje que no dice cuáles. Detectarlos antes en la propia migración con un `DO $$ ... RAISE EXCEPTION $$` que los liste (patrón en `f2d5a81c9e37`).
- **Renumerar una columna que está en una `UNIQUE` no DEFERRABLE**: el ORM emite un UPDATE por fila y la unicidad se comprueba fila a fila, así que permutar valores (2→1, 1→2) revienta a mitad de camino aunque el estado final sea válido. Dos fases: desplazar todo fuera del rango en uso (`+1000`) y después repartir los definitivos.
- En `text()` de SQLAlchemy, **`:param::tipo` no compila**: el parser lee el `::` pegado al parámetro bindeado como el comienzo de otro parámetro y falla con `syntax error at or near ":"`. Usar `CAST(:param AS date)`. Los `::` sin parámetro delante (`d::date`, `ca.severity::text`) van bien.
- **SQLAlchemy no ordena los INSERT si no hay `relationship` declarada.** En un seed que hace `db.add(Student)` y `db.add(Guardian)` en el mismo flush, el ORM no conoce la dependencia (las FKs están en la BD, no en el modelo) y puede insertar el hijo primero → `ForeignKeyViolationError`. Hacer `flush()` de los padres antes de añadir los hijos.
- **El autoflush de SQLAlchemy escribe objetos a medio mutar.** Un `SELECT` cualquiera (incluido un `max(...)` para calcular el valor siguiente) dispara flush de lo que haya sucio en la sesión: si ya cambiaste un campo de la constraint y aún no el otro, esa escritura intermedia viola la UNIQUE y el request muere con un `500` sin relación aparente. Pedir los valores derivados **antes** de mutar la entidad. Costó un 500 en `update_class_period` al mover un bloque de día.
...
- **`550 Too many emails per second` de Mailtrap puede no ser un problema de ritmo.** El plan sandbox gratuito tiene una cuota que, al agotarse, devuelve ese mismo error aunque se envíe **un correo cada 8 segundos** (medido). Antes de tocar espaciados y reintentos por ese mensaje, comprobar el ritmo real con el adaptador desnudo (`_build_provider()`, sin el wrapper de reintento) a varios intervalos: si falla igual a 8s, es cuota y no tasa.
